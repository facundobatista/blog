...
No es la `primera vez </posts/0007>`_ que menciono a `Miguel <https://tirania.org/blog/>`_ o `Nat <http://nat.org>`__, pero quiero destacar algo que me llama la atención.
...
Agregué en la barra de enlaces, en Frecuentes (a la derecha abajo), el blog de `Nat <http://nat.org/>`__. Y aprovechando que Laura finalmente se decidió por la dirección final, punteros tanto a su `blog <http://lalyum.blogspot.com/>`_ como también a su flog (EDITADO: `la url no existe más <http://www.fotolog.net/lalyum/>`__).
